...
/*   Updated: 2023/07/30 17:16:23 by hstanton         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	**expander(t_tools *tools, char **str)
{
	...
	while (str[i] != NULL)
	{
		if (str[i][detect_dollar_sign(str[i]) - 2] != '\''
			&& detect_dollar_sign(str[i]) != 0
			&& str[i][detect_dollar_sign(str[i])] != '\0')
		{
...
char	*expander_str(t_tools *tools, char *str)
{
	...
	tmp = NULL;
	if (str[detect_dollar_sign(str) - 2] != '\"' && detect_dollar_sign(str) != 0
		&& str[detect_dollar_sign(str)] != '\0')
	{
